...
u32 SetupCommonKeyY0x3D(u32 commonKeyIndex)
{
    ...
    static const u8 common_keyy_panda[6][16] = {
    ...
    bool pandaKeys = (GetUnitKeysType() == KEYS_PANDA);
    
    if (commonKeyIndex >= 6)
        return 1;
    setup_aeskeyY(0x3D, (void*) (pandaKeys ? common_keyy_panda : common_keyy)[commonKeyIndex]);
    use_aeskey(0x3D);
